...
/*
 * Approximate the boundary of a convex polytope. 
 *
 * **Authors:**
...
int main(int argc, char** argv)
{
    ...
    bool simplify = false;
    int max_edges = 0; 
    if (argc == 2)    // Only acceptable call here is "./getBoundary --help"
    {
        std::string option(argv[1]);
        if (option == "-h" || option == "--help")
        {
            std::stringstream ss;
            ss << "\nCommand:\n\t./getBoundary [-s NUM_EDGES] INPUT OUTPUT\n\n";
    ...
    else if (argc == 3)    // Only acceptable call here is "./getBoundary INPUT OUTPUT"
    {
        infilename = argv[1];
        outprefix = argv[2];
    }
    ...
    else if (argc == 5)    // Only acceptable call here is "./getBoundary -s NUM_EDGES INPUT OUTPUT"
    {
        std::string option(argv[1]);
        if (option == "-s")
        {
